...
export function formatAndPrintRange(result: RangeAnalysisResult) {
  ...
  console.log(`区间: ${r.low.toFixed(2)} - ${r.high.toFixed(2)} | 收缩强度: ${result.compressionScore}/100`);
  ...
  if (result.breakout) {
    ...
    console.log(`成交量扩张: ${b.volumeExpansion ? '是' : '否'} | 延续: ${b.followThrough ? '是' : '否'} | 回测: ${b.retested ? '是' : '否'}`);

    // 简要策略分支
    if (b.qualityScore >= 70) {
      console.log(`建议: 高质量突破，回踩 ${b.direction === 'up' ? r.high.toFixed(2) : r.low.toFixed(2)} 后顺势介入`);
