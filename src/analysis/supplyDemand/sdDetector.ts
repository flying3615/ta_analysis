...
import type { SdAnalysisResult, Zone, ZoneStatus, ZoneType } from './sdTypes.js';
...
function detectBaseZones(data: Candle[], type: ZoneType): Zone[] {
  ...
  for (let i = sdConfig.baseWindow; i < data.length - sdConfig.impulseWindow; i++) {
    ...
    if (type === 'demand' && nextMoveUp > atr * sdConfig.impulseAtrMultiplier) {
      zones.push({ type, timeframe: 'daily', startIndex: i - sdConfig.baseWindow, endIndex: i, low, high, status: 'fresh' });
    }
    if (type === 'supply' && nextMoveUp < -atr * sdConfig.impulseAtrMultiplier) {
      zones.push({ type, timeframe: 'daily', startIndex: i - sdConfig.baseWindow, endIndex: i, low, high, status: 'fresh' });
    }
  }
  return zones;
...
function updateZoneStatus(data: Candle[], zones: Zone[]): Zone[] {
  ...
  return zones.map(z => {
    ...
    // 测试（触达）
    const touched = z.type === 'demand' ? current <= z.high && current >= z.low : current >= z.low && current <= z.high;
    if (touched && z.status === 'fresh') return { ...z, status: 'tested' as ZoneStatus, lastTouchIndex: data.length - 1 };
    return z;
  });
}
...
export function analyzeSupplyDemand(
  ...
  let zones = updateZoneStatus(data, [...demandZones, ...supplyZones])
    .filter(z => (z.high - z.low) / z.low >= sdConfig.minZoneWidthPercent);

  const recentEffectiveZones = zones.filter(z => z.status !== 'broken').slice(-5);
