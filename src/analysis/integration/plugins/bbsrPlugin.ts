...
export function createBbsrPlugin(): AnalyzerPlugin {
  return {
    ...
    extract(input: AnalysisInputData, _context: IntegrationContext): DirectionConversionResult {
      ...
      if (cand.length === 0) return { direction: TradeDirection.Neutral, confidence: 0, source: 'bbsr' };
      const now = Date.now();
      ...
      for (const s of cand) {
        ...
        const score = clamp(0.6 * (s.strength ?? 0) + proximityScore + recencyScore);
        if (score > best.score) best = { direction: dir, score };
      }
      if (daily?.signal?.patternType && weekly?.signal?.patternType && daily.signal.patternType !== weekly.signal.patternType) {
