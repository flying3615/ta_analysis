...
export function createVolumePlugin(): AnalyzerPlugin {
  return {
    ...
    extract(input: AnalysisInputData, _context: IntegrationContext): DirectionConversionResult {
      const v = (input.analyses.volatility as any).volumeAnalysis.volumeAnalysis;
      ...
      if (v.divergence?.type === 'bullish' || v.divergence?.type === 'hidden_bullish') direction = TradeDirection.Long;
      if (v.divergence?.type === 'bearish' || v.divergence?.type === 'hidden_bearish') direction = TradeDirection.Short;
      ...
      if (typeof v.moneyFlowIndex === 'number') {
        if (v.moneyFlowIndex > 80) confidence -= 5; else if (v.moneyFlowIndex < 20) confidence += 5;
      }
