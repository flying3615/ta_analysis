...
export function createPatternPlugin(): AnalyzerPlugin {
  return {
    ...
    extract(input: AnalysisInputData, _context: IntegrationContext): DirectionConversionResult {
      ...
      const direction = dir === PatternDirection.Bullish ? TradeDirection.Long : dir === PatternDirection.Bearish ? TradeDirection.Short : TradeDirection.Neutral;
      const tfWeights: Record<'weekly'|'daily'|'1hour', number> = { weekly: 0.4, daily: 0.4, '1hour': 0.2 };
      let statusAdj = 0;
