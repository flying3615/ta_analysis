...
import type { IntegrationConfig, IntegrationWeights } from './IntegrationConfig.js';
...
export class SignalAggregator {
  ...
  aggregateSignals(
    ...
    // 附加分析模块
    const structureResult = this.extractStructureSignal(input.analyses.structure);
    const supplyDemandResult = this.extractSupplyDemandSignal(input.analyses.supplyDemand);
    const rangeResult = this.extractRangeSignal(input.analyses.range);
    const trendlineResult = this.extractTrendlineSignal(input.analyses.trendline);

    // 计算加权分数
    const chipWeighted = this.calculateWeightedScore(chipResult, normalizedWeights.chip);
    const patternWeighted = this.calculateWeightedScore(patternResult, normalizedWeights.pattern);
    const volumeWeighted = this.calculateWeightedScore(volumeResult, normalizedWeights.volume);
    const bbsrWeighted = this.calculateWeightedScore(bbsrResult, normalizedWeights.bbsr);
    
    // 附加权重（不计入基础100%）
    const structureWeighted = this.calculateAdditionalScore(structureResult, normalizedWeights.structure || 0);
    const supplyDemandWeighted = this.calculateAdditionalScore(supplyDemandResult, normalizedWeights.supplyDemand || 0);
    const rangeWeighted = this.calculateAdditionalScore(rangeResult, normalizedWeights.range || 0);
    const trendlineWeighted = this.calculateAdditionalScore(trendlineResult, normalizedWeights.trendline || 0);

    // 计算最终分数
    const finalScore = chipWeighted.weightedScore + 
    ...
    return {
      ...
      contributions: {
        ...
        supplyDemand: supplyDemandWeighted / (normalizedWeights.supplyDemand || 1),
   ...
   * 提取筹码分析信号
   */
  private extractChipSignal(chipAnalysis: AnalysisInputData['analyses']['chip']): DirectionConversionResult {
   ...
   * 提取形态分析信号
   */
  private extractPatternSignal(patternAnalysis: AnalysisInputData['analyses']['pattern']): DirectionConversionResult {
    const direction = this.convertPatternDirection(patternAnalysis.combinedSignal);
   ...
   * 提取波动率/成交量信号
   */
  private extractVolumeSignal(volatilityAnalysis: AnalysisInputData['analyses']['volatility']): DirectionConversionResult {
   ...
   * 提取BBSR信号
   */
  private extractBBSRSignal(bbsrAnalysis: AnalysisInputData['analyses']['bbsr']): DirectionConversionResult {
    // 根据BBSR分析结果提取方向信号
    let direction = TradeDirection.Neutral;
    let confidence = 0;
    
    // 检查日线和周线BBSR结果
    const signals = [bbsrAnalysis.dailyBBSRResult, bbsrAnalysis.weeklyBBSRResult].filter(Boolean);
    
    for (const signal of signals) {
      if (signal && signal.strength > confidence) {
        // 简化处理：基于强度判断方向
        direction = signal.strength > 50 ? TradeDirection.Long : TradeDirection.Short;
   ...
   * 提取结构分析信号
   */
  private extractStructureSignal(structureAnalysis: AnalysisInputData['analyses']['structure']): DirectionConversionResult {
    let direction = TradeDirection.Neutral;
    let confidence = 50;
   ...
   * 提取供需分析信号
   */
  private extractSupplyDemandSignal(sdAnalysis: AnalysisInputData['analyses']['supplyDemand']): DirectionConversionResult {
    let direction = TradeDirection.Neutral;
    let confidence = 50;
    ...
    // 考虑有效供需区域
    const freshZones = sdAnalysis.recentEffectiveZones?.filter(zone => zone.status === 'fresh') || [];
   ...
   * 提取区间突破分析信号
   */
  private extractRangeSignal(rangeAnalysis: AnalysisInputData['analyses']['range']): DirectionConversionResult {
    let direction = TradeDirection.Neutral;
    let confidence = 50;
    ...
    if (rangeAnalysis.breakout) {
      ...
      // 如果有回踩确认，提高置信度
      if (rangeAnalysis.breakout.retested && rangeAnalysis.breakout.followThrough) {
   ...
   * 提取趋势线分析信号
   */
  private extractTrendlineSignal(trendlineAnalysis: AnalysisInputData['analyses']['trendline']): DirectionConversionResult {
    let direction = TradeDirection.Neutral;
    let confidence = 50;
  ...
  private calculateWeightedScore(
    ...
    const directionMultiplier = signal.direction === TradeDirection.Long ? 1 :
                               signal.direction === TradeDirection.Short ? -1 : 0;
   ...
   * 转换形态方向
   */
  private convertPatternDirection(patternDirection: PatternDirection): TradeDirection {
  ...
  private determineSignalStrength(
    ...
    // 考虑筹码和形态方向的一致性
    const directionsAlign = chipDirection === patternDirection && chipDirection !== TradeDirection.Neutral;
    ...
    } else if (absScore > this.config.thresholds.volatilityAdjustedScoreModerate) {
  ...
  private calculateConfidenceScore(
    ...
    // 根据波动率调整置信度（恢复原有逻辑）
    const volatilityScore = this.calculateVolatilitySignalStrength(volatilityAnalysis);
   ...
   * 计算波动率信号强度（恢复原有逻辑）
   */
  private calculateVolatilitySignalStrength(volatilityAnalysis: AnalysisInputData['analyses']['volatility']): number {
    // 提取波动率分析数据（修正结构）
    const volAnalysis = volatilityAnalysis.volatilityAnalysis.volatilityAnalysis;
    const volPriceConfirmation = volatilityAnalysis.volumeAnalysis.volumeAnalysis;
