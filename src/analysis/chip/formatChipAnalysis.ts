...
export function formatAndPrintChipAnalysis(
  ...
  const shortBarLength = Math.round(analysisResult.combinedShortSignalStrength / 5);
  const buyBar = '█'.repeat(buyBarLength) + '░'.repeat(20 - buyBarLength);
  const shortBar = '█'.repeat(shortBarLength) + '░'.repeat(20 - shortBarLength);
  console.log(`多头信号: ${buyBar} (${analysisResult.combinedBuySignalStrength}/100)`);
  console.log(`空头信号: ${shortBar} (${analysisResult.combinedShortSignalStrength}/100)`);
  console.log(`信号差值: ${Math.abs(analysisResult.combinedBuySignalStrength - analysisResult.combinedShortSignalStrength)}`);
  console.log(`时间周期一致性: ${analysisResult.timeframeAlignment} (${analysisResult.alignmentStrength}%)`);
  ...
  analysisResult.timeframes.forEach(tf => {
    const timeframeName = tf.timeframe === 'weekly' ? '周线     ' : tf.timeframe === 'daily' ? '日线     ' : '小时线   ';
    ...
    const diffStrength = Math.abs(tf.analysis.buySignalStrength - tf.analysis.shortSignalStrength).toString().padEnd(4);
  ...
  if (analysisResult.stopLossLevels.length > 0) {
    console.log(`止损位: ${analysisResult.stopLossLevels.map(level => level.toFixed(2)).join(', ')}`);
  } else {
    console.log('无明确止损位建议');
  }
  if (analysisResult.takeProfitLevels.length > 0) {
    console.log(`止盈位: ${analysisResult.takeProfitLevels.map(level => level.toFixed(2)).join(', ')}`);
  } else {
  ...
  if (analysisResult.aggregatedSupportLevels.length > 0) {
    console.log(`支撑位: ${analysisResult.aggregatedSupportLevels.map(level => level.toFixed(2)).join(', ')}`);
  } else {
    console.log('未检测到明确的支撑位');
  }
  if (analysisResult.aggregatedResistanceLevels.length > 0) {
    console.log(`阻力位: ${analysisResult.aggregatedResistanceLevels.map(level => level.toFixed(2)).join(', ')}`);
  } else {
