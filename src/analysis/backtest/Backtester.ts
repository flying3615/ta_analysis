...
function calcSharpe(returns: number[]): number {
	...
	const varr = returns.reduce((s, r) => s + Math.pow(r - mean, 2), 0) / returns.length;
...
export class Backtester {
	...
	run(candles: Candle[], strategy: Strategy): BacktestResult {
		...
		for (let i = 50; i < candles.length - 1; i++) {
			...
			stepReturns.push(equity.length > 1 ? (equity[equity.length - 1] - equity[equity.length - 2]) / equity[equity.length - 2] : 0);
