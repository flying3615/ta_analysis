import { analyzeMultiTimeBBSR, formatAndPrintSrAnalysis, getStockDataForTimeframe } from '../index.js';
...
async function main() {
  ...
  const startWeekly = new Date(today); startWeekly.setDate(today.getDate() - 365);
  const startDaily = new Date(today); startDaily.setDate(today.getDate() - 120);
