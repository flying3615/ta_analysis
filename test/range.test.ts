...
function makeRangeBreakoutData(): Candle[] {
	...
	for (let i = 0; i < 30; i++) {
		out.push({ symbol: 'R', open: 100, high: 100.5, low: 99.5, close: 100, volume: 1000, timestamp: new Date(t) });
		t += day;
	}
	// breakout up with expansion
	out.push({ symbol: 'R', open: 100.6, high: 105, low: 100.6, close: 104, volume: 4000, timestamp: new Date(t) });
	t += day;
	// optional retest near high boundary then continue
	out.push({ symbol: 'R', open: 103, high: 104, low: 100.5, close: 102.5, volume: 2000, timestamp: new Date(t) });
	t += day;
	// follow through
	out.push({ symbol: 'R', open: 102.6, high: 106, low: 102, close: 105, volume: 2500, timestamp: new Date(t) });
	return out;
}
